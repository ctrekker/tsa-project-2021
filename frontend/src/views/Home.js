import react, {useState, useEffect} from "react";
import {Divider, Grid, IconButton, Typography, Container} from "@material-ui/core";
import ClassPreviewList from "../components/ClassPreviewList";
import LobbyPreviewList from "../components/LobbyPreviewList";
import moment from "moment";
import {ArrowRight, ArrowLeft} from "@material-ui/icons";


const classPreviews = [
    {title:"Basics of Derivatives", author:"Connor Burns", rating:'4.3/5', scheduledFor:moment("2/8/2021 7:30")},
    {title:"Mistakes in Calculus 10th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 6:00')},
    {title:"Mistakes in Calculus 21th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 8:30')},
    {title:"Mistakes in Calculus 21th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 8:30')},
    {title:"Mistakes in Calculus 21th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 8:30')},
    {title:"Mistakes in Calculus 21th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 8:30')},
    {title:"Mistakes in Calculus 21th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 8:30')},
    {title:"Mistakes in Calculus 21th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 8:30')},
    {title:"Mistakes in Calculus 21th Edition", author:"Todd Mortley", rating:'6.28/5', scheduledFor:moment('2/11/2021 8:30')}
];

const lobbies = [
    { name: 'Algebra', category: "Math", description: "Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni", picture: "data:image/jpeg;base64,",
classCount: 500, memberCount: 341, postCount: 547, rating: 4.3},
    { name: 'Geometry', category: "Math", description: "This is a place to learn geometry", picture: "data:image/jpeg;base64,",
classCount: 24, memberCount: 3412, postCount: 1547, rating: 3.8},
    { name: 'Calculus', lobbyId: 1 },
    { name: 'Linear Algebra' },
    { name: 'Complex Analysis' },
    { name: 'Combinatorics' },
    { name: 'Topology' },
    { name: 'Biology' },
    { name: 'Chemistry' },
    { name: 'Physics' },
    { name: 'Environmental Science' },
    { name: 'United States History' },
    { name: 'World History' },
    { name: 'European History' },
    { name: 'Spanish', category: "Foreign Language"}
]

export default function Home(){
    // TODO Clean up this mess and possibly add smooth transition
    // TODO Fix Jank

    //When the home bar is added the class List sidebar inline css will have to be modefied. It currently uses fixed position
    const [showClasses, setShowClasses] = useState({borderLeft: '1px solid lightgrey', position: "fixed", top: 0, right: 0});
    const [margin, setMargin] = useState({});
    const [showButton, setShowButton] = useState({display: "none"});

    useEffect(() => {
        function handleScroll(e) {
            if(window.scrollY > 1 && showClasses.borderLeft) {
                setShowClasses({display:"none"});
                setMargin({marginLeft: "10%"});
                setShowButton({display: "inherit", position: "fixed", top: "40%", right: 0, fontSize: "4em"})
            }
            else if(window.scrollY == 0 && !showClasses.borderLeft) {
                setShowClasses({borderLeft: '1px solid lightgrey'});
                setMargin({});
                setShowButton({display: "none"})
            }
        }

        window.addEventListener('scroll', handleScroll);

        return () => {
            window.removeEventListener('scroll', handleScroll);
        }
    }, [showClasses, setShowClasses]);

    return (
        <div style={{display:"flex", flexDirection:"row", marginLeft: "5%"}}>
            <Grid item sm={6} md={8} lg={9} style ={margin}>
                <LobbyPreviewList lobbies={lobbies}/>
            </Grid>
            <Grid item xs="4" style={showClasses}>
                <div style={{display:"flex", flexDirection:"column", padding: 15, position: "sticky", overflowX: "hidden"}}>
                    <div style={{display:"flex", flexDirection:"row"}}>
                        <IconButton component={ArrowRight} onClick={() => {setShowClasses({display: "none"}); setMargin({marginLeft: "10%"}); setShowButton({display: "inherit", position: "fixed", top: "40%", right: 0, fontSize: "4em"})}} style={{fontSize:"2em"}}/>
                        <Typography variant="h4" style={{marginTop:7}}>Upcoming Classes</Typography>
                    </div>
                    <Divider/>
                    <div style={{maxHeight: "85vh", overflowY: "auto", marginRight: -50, paddingRight: 50}}>
                        <ClassPreviewList classPreviews={classPreviews}/>
                    </div>
                </div>
            </Grid>
            <IconButton component = {ArrowLeft} onClick={() => {setShowClasses({borderLeft: '1px solid lightgrey', position: "fixed", top: 0, right: 0}); setMargin({paddingRight: "10vw"}); setShowButton({display: "none"})}} style={showButton}/>
        </div>
    );
}
